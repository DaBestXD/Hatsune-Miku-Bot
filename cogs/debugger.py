...
from botextras.bot_funcs_ext import owner_command, reply
from botextras.audio_handler import get_Song_Info
...
class botDebugger(commands.Cog):
        ...
        for n in self.bot.cogs.keys():
            ...
            if bot_cog:
                for c in bot_cog.get_app_commands():
                    print(c)
        await reply(interaction, "Commands")
        return None
    ...
    async def reload_cog(self, interaction: Interaction, cog_name: str):
        ...
        if strcog in self.bot.extensions:
            ...
            await reply(interaction, f"`Reloading {strcog}`")
        else:
            strsli :list[str] = []
            strsli.append("```\n")
    ...
    async def die(self, interaction: discord.Interaction) -> None:
        if not USER_ID:
            await reply(interaction, "`User ID was never provided`", ephemeral=True)
            return None
        if interaction.user.id == USER_ID:
            await reply(interaction, "`Shutting down...`")
            await self.bot.close()
            return None
        await reply(interaction, "Not allowed", ephemeral=True)
        return None
