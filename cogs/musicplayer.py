...
from botextras.bot_funcs_ext import reply, build_audio, return_general_embed
...
class GuildPlaybackState():
    ...
    async def cache_index(self, idx: int = 1) -> bool:
        ...
        if song.title and song.webpage_url:
    ...
    async def helper_play_next(self, g_id: int, bot: commands.Bot, failed: bool = False):
        ...
        if self.text_channel and not self.songs_list:
            await self.text_channel.send(embed=return_general_embed("Queue empty"))
            self.source = None
        if self.song_loop:
...
class reMikuMusicCommands(commands.Cog):
    ...
    async def join_vc(self, interaction: discord.Interaction) -> None | VoiceProtocol:
        ...
        if user.voice and user.voice.channel:
            ...
            await reply(interaction,embed=return_general_embed("Join a voice channel first!"), ephemeral=True)
            return None
    ...
    async def play(self, interaction: discord.Interaction, song_name: str) -> None:
        ...
        if not song_info:
            await reply(interaction, embed=return_general_embed("Invalid link or song"))
            return None
        ...
        if ffmpeg_source:
            ...
            await reply(interaction, embed=return_general_embed("Something went wrong... try again!"))
        return None
    ...
    async def queue(self, interaction: discord.Interaction) -> None:
        ...
        if not gp_state.songs_list:
            await reply(interaction, embed=return_general_embed("Queue empty"))
            return None
    ...
    async def skip(self, interaction: discord.Interaction) -> None:
        ...
        gp_state = self.guildpback_dict[g_id]
        if not gp_state.vc:
            await reply(interaction, embed=return_general_embed("Not in a voice channel"), ephemeral=True)
            return None
        if not gp_state.songs_list:
            await reply(interaction, embed=return_general_embed("Queue empty!"))
            return None
    ...
    async def shuffle(self, interaction: discord.Interaction) -> None:
        ...
        await reply(interaction, embed=return_general_embed("Queue shuffled"))
        if exl_first:
            await gp_state.override_task()
        return None
    ...
    async def loopSong(self, interaction: discord.Interaction) -> None:
        ...
        if not gp_state.song_loop:
            ...
            await reply(interaction, embed=return_general_embed("Looping current song!"))
            return None
        gp_state.song_loop = False
        await reply(interaction, embed=return_general_embed("No longer looping current song!"))
        return None
    ...
    async def removeFromQueue(self, interaction: discord.Interaction, index: int) -> None:
        ...
        try:
            ...
            await reply(interaction, embed=return_general_embed("Not a valid number!"), ephemeral=True)
        return None
    ...
    async def stop(self, interaction: discord.Interaction) -> None:
        ...
        gp_state = self.guildpback_dict[g_id]
        if not gp_state.vc:
            await reply(interaction, embed=return_general_embed("Not in a voice channel!"))
            return None
        await gp_state.voice_cleanup(leave_vc=True)
        await reply(interaction, embed=return_general_embed("Stopping playblack..."))
        return None
    ...
    async def clear(self, interaction: discord.Interaction) -> None:
        ...
        if guild.voice_client and gp_state.vc:
            gp_state.vc.stop()
            gp_state.songs_list = []
            await reply(interaction,embed=return_general_embed("Clearing queue..."))
            return None
        else:
            await reply(interaction,embed=return_general_embed("Not in a voice channel"), ephemeral=True)
            return None
    ...
    async def set_volume(self, interaction: discord.Interaction, volume: float) -> None:
        ...
        if gp_state.source:
            ...
            await reply(interaction, embed=return_general_embed(f"Set volume to {volume}!"))
            return None
        await reply(interaction, embed=return_general_embed("No audio source found!"), ephemeral=True)
        return None
