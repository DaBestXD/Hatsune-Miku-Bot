...
async def join_vc(interaction: Interaction, join:bool = True) -> None|VoiceProtocol:
    ...
    if user.voice and user.voice.channel:
        ...
        await reply(interaction,embed=text_only_embed("Join a voice channel first!"), ephemeral=True)
        return None

async def mod_song(mod_type: Literal["pitch","speed","bass","off"], effect_strength: float = 0) -> str:
    song_mods = ""
    if mod_type == "pitch":
